...
### 5.1. MonoBehaviour 가이드라인

* **GetComponent 캐싱:** `Update()`에서 `GetComponent<T>()`를 호출하지 않습니다. `Awake()` 또는 `Start()`에서 캐싱합니다.
* **Find 메서드 지양:** `GameObject.Find()`, `FindObjectOfType<T>()`는 성능 이슈가 있으므로 가급적 사용하지 않습니다.
* **Null 체크:** `UnityEngine.Object`의 null 체크는 `== null` 또는 `!= null`을 사용합니다.

### 5.2. 성능 최적화

* **Object Pooling:** 자주 생성/파괴되는 오브젝트는 오브젝트 풀링을 사용합니다.
* **String 연산:** `Update()`에서 문자열 연결을 피합니다.
* **박싱 방지:** 값 타입의 박싱을 최소화합니다.
